...
- [x] **Stage 11 — Deployment**: deployed on Render free tier via `render.yaml` blueprint. Public URL: https://grid-guardian-7djv.onrender.com. Verified live: real hazard count, real risk scoring, and real Groq LLM briefing all confirmed working against the public deployment (not just localhost).
- [x] **Stage 12 — Documentation**: README with architecture, live demo link, setup, deployment, and an honest known-limitations section. Along the way, found and fixed a real issue caught by live-testing the public deployment: the shared demo org's token was publicly visible in frontend JS, making its portfolio writable by anyone — patched to reject writes with a 403, verified live.
